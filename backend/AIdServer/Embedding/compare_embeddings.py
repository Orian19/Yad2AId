from backend.AIdServer.Embedding.Embbed import create_connection
import numpy as np 
  ...
  # Adjust table and column names if necessary
    ...
    for row in result:
        ...
        if embedding.shape == expected_shape:
            ...
            print(f"Warning: Embedding shape {embedding.shape} for Apartment ID {row[0]} does not match expected {expected_shape}")
    con.close()
